...
def filter_matches(kp1, kp2, matches,desc1, desc2, ratio = 0.75):
    mkp1, mkp2 = [], []
    mdesc1, mdesc2=[],[]
    for m in matches:
        if len(m) == 2 and m[0].distance < m[1].distance * ratio:
            ...
            mdesc1.append(desc1[m.queryIdx])
            mdesc2.append(desc2[m.trainIdx])
    p1 = np.float32([kp.pt for kp in mkp1])
    p2 = np.float32([kp.pt for kp in mkp2])
    kp_pairs = zip(mkp1, mkp2)
    desc_pairs = zip(mdesc1, mdesc1)
    return p1, p2, list(kp_pairs), list(desc_pairs)
